...
-/
import LeanMath.EmbeddingDimension
...
* `IsLocalRing.Quotient_singleton_ringKrullDim`: If `x ∈ m` then `ringKrullDim R ≤ 1 + ringKrullDim R ⧸ x`
* `IsLocalRing.ringKrullDim_le_embDim`: The embedding dimension of `R` is an upper bound on the Krull Dimension of `R`
* `IsLocalRing.ringKrullDim_quotient_succ_eq_of_nonZeroDivisor`: If `x` is a non-unit, non-zerodivisor then
...
instance {R : Type*} [CommRing R] [IsLocalRing R] [IsNoetherianRing R] :
  ...
  have : Submodule.spanFinrank (maxl R) < (⊤ : ℕ∞) := ENat.coe_lt_top (Submodule.spanFinrank maxl R)
  by_contra hc
...
theorem IsLocalRing.exists_finset_card_eq_ringKrullDim (R : Type*) [CommRing R] [IsLocalRing R] [IsNoetherianRing R] :
  ...
  rw[b, ← hs2] at a
  have : s.card = finRingKrullDim R := (@Nat.cast_inj (WithBot ℕ∞) _ _ s.card (finRingKrullDim R)).mp a
  use s
  rw[this]
...
[Nontrivial (R ⧸ (Ideal.span {x}))] : ringKrullDim R ≤ ringKrullDim (R ⧸ (Ideal.span {x})) + 1 := by
  ...
  have maxmin : (maxl R) ∈ (span S').minimalPrimes := by
    have : (maxl R) = comap (Ideal.Quotient.mk (span {x})) (maxl (R ⧸ span {x})) := Eq.symm (IsLocalRing.Quotient_comap_maximalIdeal (span {x}))
    ...
    exact this
  have KDRle := (Ideal.height_le_spanRank_toENat_of_mem_minimal_primes (span (S' : Set R)) (maxl R)) maxmin
  ...
  have maxleK : (maxl R).height ≤ finRingKrullDim (R ⧸ span {x}) + 1 := by
    ...
    exact le_trans a c
  have al : ((maxl R).height : WithBot ℕ∞) ≤ finRingKrullDim (R ⧸ span {x}) + 1 := (WithBot.coe_le rfl).mpr maxleK
  rw[maxKDR, ← finRingKrullDim_ringKrullDim (R ⧸ span {x})] at al
  exact al

theorem IsLocalRing.Quotient_span_singleton_spanFinrank
[IsLocalRing R] [IsNoetherianRing R] (x : R) [Nontrivial (R ⧸ (Ideal.span {x}))] (hx1 : x ∈ (maxl R)) (hx2 : x ∉ (maxl R)^2) :
    Submodule.spanFinrank (maxl (R ⧸ Ideal.span {x})) + 1 = Submodule.spanFinrank (maxl R) := by
  rw[← IsLocalRing.embDim_eq_spanFinrank_maximalIdeal R, ← IsLocalRing.embDim_eq_spanFinrank_maximalIdeal (R ⧸ Ideal.span {x})]
...
theorem IsLocalRing.ringKrullDim_le_embDim (R : Type*) [CommRing R] [IsLocalRing R] [IsNoetherianRing R] :
